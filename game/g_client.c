...
void SP_info_player_duel1(gentity_t* ent)
{
    ...
    if (g_gametype.integer == GT_FFA)
    {
        if (disablesenabled && thereIsAPlayerSpawn())
        {
            //Lugormod
            if (ent->target && ent->target[0])
            {
...
const entityInfoData_t info_player_deathmatch_spawnflags[] = {
    {
        "1",
        "Initial: The first time the player enters the game (from connection or a team switch), they will spawn here."
    },
    {NULL, NULL}
};
...
const entityInfoData_t info_player_siegeteam1_keys[] = {
    ...
    {
        "idealclass",
        "if specified, this spawn point will be considered \'ideal\' for players of this class. Corresponds to the name entry in the .scl (siege class) file"
    },
    ...
    while (i < MAX_SABERS)
    {
        if (ent->client->saber[i].model[0])
        {
            ...
            if (ent->client->weaponGhoul2[i])
            {
                ...
                {
                    trap_G2API_SetSkin(ent->client->weaponGhoul2[i], 0, ent->client->saber[i].skin,
...
void SetupGameGhoul2Model(gentity_t* ent, char* modelname, char* skinName)
{
    ...
    if (precachedKyle && trap_G2_HaveWeGhoul2Models(precachedKyle))
    {
            ...
            G_PlayerHasCustomSkeleton(ent))
        {
            ...
            if (ent->client->NPC_class == CLASS_VEHICLE)
            {
                ...
                {
                    skinHandle = trap_R_RegisterSkin(va("models/players/%s/model_%s.skin", modelname,
                                                        ent->m_pVehicle->m_pVehicleInfo->skin));
                }
                else
                {
        ...
        ent->m_pVehicle)
    {
        ...
        for (i = 0; i < MAX_VEHICLE_TURRET_MUZZLES; i++)
        {
            ...
            {
                ent->m_pVehicle->m_iGunnerViewTag[i] = trap_G2API_AddBolt(
    ...
    if (ent->client->ps.weapon == WP_SABER || ent->s.number < MAX_CLIENTS)
    {
        ...
        trap_G2API_SetBoneAngles(ent->ghoul2, 0, "upper_lumbar", tempVec, BONE_ANGLES_POSTMULT, POSITIVE_X, NEGATIVE_Y,
                                 NEGATIVE_Z, NULL, 0, level.time);
        trap_G2API_SetBoneAngles(ent->ghoul2, 0, "cranium", tempVec, BONE_ANGLES_POSTMULT, POSITIVE_Z, NEGATIVE_Y,
...
void ClientUserinfoChanged_Do(int clientNum)
{
    ...
    */
    ...
    {
        s = va(
            "n\\%s\\t\\%i\\model\\%s\\c1\\%s\\c2\\%s\\c3\\%s\\c4\\%s\\hc\\%i\\w\\%i\\l\\%i\\skill\\%s\\tt\\%d\\tl\\%d\\siegeclass\\%s\\st\\%s\\st2\\%s\\dt\\%i\\sdt\\%i",
            client->pers.netname, team, model, c1, c2, cp_sbRGB1, cp_sbRGB2,
            client->pers.maxHealth, client->sess.wins, client->sess.losses,
            Info_ValueForKey(userinfo, "skill"), teamTask, teamLeader, className, saberName, saber2Name,
            client->sess.duelTeam, client->sess.siegeDesiredTeam);
    }
    else
    {
        if (g_gametype.integer == GT_SIEGE
            || g_gametype.integer == GT_BATTLE_GROUND) //Lugormod
        {
            //more crap to send
            s = va(
                "n\\%s\\t\\%i\\model\\%s\\c1\\%s\\c2\\%s\\c3\\%s\\c4\\%s\\hc\\%i\\w\\%i\\l\\%i\\tt\\%d\\tl\\%d\\siegeclass\\%s\\st\\%s\\st2\\%s\\dt\\%i\\sdt\\%i",
                client->pers.netname, client->sess.sessionTeam, model, c1, c2, cp_sbRGB1, cp_sbRGB2,
                client->pers.maxHealth, client->sess.wins, client->sess.losses, teamTask, teamLeader, className,
                saberName, saber2Name, client->sess.duelTeam, client->sess.siegeDesiredTeam);
        }
        else
        {
            s = va(
                "n\\%s\\t\\%i\\model\\%s\\c1\\%s\\c2\\%s\\c3\\%s\\c4\\%s\\hc\\%i\\w\\%i\\l\\%i\\tt\\%d\\tl\\%d\\st\\%s\\st2\\%s\\dt\\%i",
                client->pers.netname, client->sess.sessionTeam, model, c1, c2, cp_sbRGB1, cp_sbRGB2,
                client->pers.maxHealth, client->sess.wins, client->sess.losses, teamTask, teamLeader, saberName,
                saber2Name, client->sess.duelTeam);
        }
    }
...
char* ClientConnect(int clientNum, qboolean firstTime, qboolean isBot)
{
                    ...
                    if (c > lmd_maxsameip.integer)
                    {
                        ...
                        if (lmd_autobansameip.integer)
                        {
                            ...
                            trap_SendServerCommand(
                                -1, va(
                                    "print \"^3IP ^2%s^3 has been banned from the server due to too many connections.",
                                    clientip));
                        ...
                        return
                            "Too many clients have been detected from this ip, refusing connection\0x7\0xE\0x7\0xE\0x7\0xE";
                    }
                }
            }
        }
    ...
    else if (client->sess.sessionTeam != client->sess.Lmd.forcedTeam)
    {
        client->sess.sessionTeam = client->sess.Lmd.forcedTeam;
    }
...
void G_UpdateClientAnims(gentity_t* self, float animSpeedScale)
{
    ...
    {
        trap_G2API_SetBoneAnim(self->ghoul2, 0, "model_root", self->client->ps.saberLockFrame,
                               self->client->ps.saberLockFrame + 1, BONE_ANIM_OVERRIDE_FREEZE | BONE_ANIM_BLEND,
                               animSpeedScale, level.time, -1, 150);
        trap_G2API_SetBoneAnim(self->ghoul2, 0, "lower_lumbar", self->client->ps.saberLockFrame,
                               self->client->ps.saberLockFrame + 1, BONE_ANIM_OVERRIDE_FREEZE | BONE_ANIM_BLEND,
                               animSpeedScale, level.time, -1, 150);
        trap_G2API_SetBoneAnim(self->ghoul2, 0, "Motion", self->client->ps.saberLockFrame,
                               self->client->ps.saberLockFrame + 1, BONE_ANIM_OVERRIDE_FREEZE | BONE_ANIM_BLEND,
                               animSpeedScale, level.time, -1, 150);
        return;
    }
    ...
    if (self->client->legsAnimExecute != legsAnim || self->client->legsLastFlip != self->client->ps.legsFlip)
    {
        ...
        trap_G2API_SetBoneAnim(self->ghoul2, 0, "model_root", firstFrame, lastFrame, aFlags, lAnimSpeedScale,
                               level.time, -1, 150);
...
tryTorso:
        ...
        !self->noLumbar)
    {
        ...
        BG_SaberStartTransAnim(self->s.number, self->client->ps.fd.saberAnimLevel, self->client->ps.weapon, f,
                               &animSpeedScale, self->client->ps.brokenLimbs);
        ...
        trap_G2API_SetBoneAnim(self->ghoul2, 0, "lower_lumbar", firstFrame, lastFrame, aFlags, lAnimSpeedScale,
        ...
        self->localAnimIndex <= 1)
    {
        ...
        trap_G2API_SetBoneAnim(self->ghoul2, 0, "Motion", firstFrame, lastFrame, aFlags, lAnimSpeedScale, level.time,
                               -1, 150);
    }
...
gentity_t* ClientSpawn_GetSpawnpoint(gentity_t* ent, vec3_t spawn_origin, vec3_t spawn_angles)
{
    ...
    else if (g_gametype.integer == GT_CTF || g_gametype.integer == GT_CTY || g_gametype.integer == GT_SABER_RUN ||
        g_gametype.integer == GT_GHOST || g_gametype.integer == GT_REBORN)
    {
    ...
    if (scl->powerups)
    {
        ...
        for (i = 0; i < PW_NUM_POWERUPS; i++)
        {
            if (bgSiegeClasses[client->siegeClass].powerups & (1 << i))
                client->ps.powerups[i] = level.time + Q3_INFINITE;
            //Ufo: Q3_INFINITE is insufficient for leveltime dependant stuff
        }
    }
    ...
    if (client->sess.duelTeam == DUELTEAM_LONE)
    {
        if (g_duel_fraglimit.integer)
        {
            client->ps.stats[STAT_MAX_HEALTH] = (int)(g_powerDuelStartHealth.integer -
                ((g_powerDuelStartHealth.integer - g_powerDuelEndHealth.integer) * (float)client->sess.wins / (float)
...
void ClientSpawn_JediVsMerc(gentity_t* ent, int wDisable)
{
    gclient_t* client = ent->client;
    int i;
    if (g_gametype.integer >= GT_TEAM && (client->sess.sessionTeam == TEAM_BLUE || client->sess.sessionTeam ==
        TEAM_RED))
    {
...
void ClientSpawn(gentity_t* ent)
{
            ...
            if (g_gametype.integer != GT_SIEGE && g_gametype.integer != GT_BATTLE_GROUND)
            {
                                                ...
                                                ent->client->ps.fd.saberAnimLevel))
                {
                    WP_UseFirstValidSaberStyle(&ent->client->saber[0], &ent->client->saber[1],
                                               ent->client->ps.saberHolstered,
                                               &ent->client->ps.fd.saberAnimLevel);
                    ent->client->ps.fd.saberAnimLevelBase = ent->client->saberCycleQueue = ent->client->ps.fd.
                        saberAnimLevel;
                }
            }
        }
    ...
    if (client->sess.sessionTeam != TEAM_SPECTATOR)
    {
        ...
        if (PlayerAcc_Prof_GetLevel(ent) > 0 && g_gametype.integer != GT_SIEGE && g_gametype.integer !=
            GT_BATTLE_GROUND) //Ufo: we don't want it in siege
        ...
        if (client->ps.weapon == WP_SABER && g_gametype.integer != GT_FFA)
            G_SetAnim(ent, SETANIM_BOTH, BOTH_STAND1TO2,
                      SETANIM_FLAG_OVERRIDE | SETANIM_FLAG_HOLD | SETANIM_FLAG_HOLDLESS, 0);
        else
        {
            G_SetAnim(ent, SETANIM_TORSO, TORSO_RAISEWEAP1,
                      SETANIM_FLAG_OVERRIDE | SETANIM_FLAG_HOLD | SETANIM_FLAG_HOLDLESS, 0);
    ...
    if (client->ps.stats[STAT_MAX_HEALTH] <= 100 && g_gametype.integer != GT_SIEGE && g_gametype.integer !=
        GT_BATTLE_GROUND)
        client->ps.stats[STAT_HEALTH] = (int)(client->ps.stats[STAT_MAX_HEALTH] * 1.25);
    else if (client->ps.stats[STAT_MAX_HEALTH] < 125 && g_gametype.integer != GT_SIEGE && g_gametype.integer !=
        GT_BATTLE_GROUND)
