...
static team_t TargetSetTeam_ParseTeam(const char* teamName)
{
    ...
    if (!Q_stricmp(teamName, "red") || !Q_stricmp(teamName, "r") || (teamName[0] == '1' && teamName[1] == '\0'))
    {
        return TEAM_RED;
    }

    if (!Q_stricmp(teamName, "blue") || !Q_stricmp(teamName, "b") || (teamName[0] == '2' && teamName[1] == '\0'))
    {
        return TEAM_BLUE;
    }

    if (!Q_stricmp(teamName, "spectator") || !Q_stricmp(teamName, "spectate")
        || !Q_stricmp(teamName, "spec") || !Q_stricmp(teamName, "s")
        || (teamName[0] == '3' && teamName[1] == '\0'))
    {
        return TEAM_SPECTATOR;
    }

    if (!Q_stricmp(teamName, "free") || !Q_stricmp(teamName, "ffa")
        || !Q_stricmp(teamName, "f") || (teamName[0] == '0' && teamName[1] == '\0'))
    {
        return TEAM_FREE;
    }
...
static const char* TargetSetTeam_CommandString(team_t team)
{
    switch (team)
    {
    case TEAM_FREE:
...
static void target_setteam_use(gentity_t* self, gentity_t* other, gentity_t* activator)
{
    const char* teamCommand;
    team_t team;
    gclient_t* cl;
    qboolean lockTeam = qfalse;
    int expireTime = 0;
    ...
    team = (team_t)self->genericValue1;
    teamCommand = TargetSetTeam_CommandString(team);
    ...
    {
        lockTeam = qtrue;
        expireTime = level.time + (int)(self->wait * 1000);
    }

    cl->sess.Lmd.forcedTeam = team;
    cl->sess.Lmd.forcedTeamExpires = lockTeam ? expireTime : 0;

    SetTeam(activator, (char*)teamCommand);

    if (cl->sess.sessionTeam != team)
    {
        SetTeamQuick(activator, team, qtrue);
    }

    if (!lockTeam)
    {
        cl->sess.Lmd.forcedTeam = TEAM_NUM_TEAMS;
        cl->sess.Lmd.forcedTeamExpires = 0;
    }
    else
    {
        int messageTime = expireTime;

        cl->sess.Lmd.forcedTeam = team;
        cl->sess.Lmd.forcedTeamExpires = expireTime;

        if (expireTime == -1)
        {
            Disp(activator, "^3Your team has been locked until the match ends.");
        }
        else
        {
            int remainingMs = messageTime - level.time;
            int remainingSec;
            ...
            Disp(activator, va("^3Your team has been locked for %i second%s.",
...
/*QUAKED target_setteam (1 0 0) (-8 -8 -8) (8 8 8)
Sets the activator's team and optionally locks them to it for a duration.

The wait key controls how long the player is unable to change teams. Set wait to -1 to lock them for the remainder of the match.
*/
const entityInfoData_t target_setteam_spawnflags[] = {
    {NULL, NULL}
};
const entityInfoData_t target_setteam_keys[] = {
    {"team", "Team to assign to the activator. Accepts free, red, blue, or spectator."},
    {"wait", "Duration in seconds to lock the activator to the team. Use -1 to lock for the rest of the match."},
    {"targetname", "make the trigger target this value for the entity to be used"},
    {NULL, NULL}
};
const entityInfo_t target_setteam_info = {
    "Sets the activator's team and can prevent them from switching teams for a period of time.",
    target_setteam_spawnflags,
    target_setteam_keys
};

void SP_target_setteam(gentity_t* self)
{
    const char* teamName;
    team_t team;

    if (!G_SpawnString("team", "", &teamName))
    {
        teamName = NULL;
    }

    team = TargetSetTeam_ParseTeam(teamName);
    if (team == TEAM_NUM_TEAMS)
    {
        EntitySpawnError("target_setteam requires a valid team value (free, red, blue, spectator).");
        G_FreeEntity(self);
        return;
    }

    self->genericValue1 = team;
    self->use = target_setteam_use;
}
