...
BG_field_t fields[] = {
    ...
    {"script_targetname", FOFS(script_targetname), F_LSTRING, ENTFIELD_NOLOGIC},
...
void SP_target_setteam(gentity_t* ent);
extern entityInfo_t target_setteam_info;
...
spawn_t spawnInitValues[] = {
    ...
    {"info_player_intermission_red", SP_info_player_intermission_red, Logical_True, &info_player_intermission_red_info},
    {
        ...
        &info_player_intermission_blue_info
    },
    ...
    {"target_setteam", SP_target_setteam, Logical_True, &target_setteam_info},
...
void Cmd_Entityinfo_t(gentity_t* ent, int iArg)
{
    ...
    else
    {
        ...
        {
            Disp(ent, va(
                     "^2%i^3 entities match that name.  Narrow the search down to get more information on an entity.",
                     c));
            return;
        }
        else if (c <= 0)
        ...
        if (spawn->info)
        {
            ...
            if (spawn->info->keys != NULL)
            {
                ...
                for (data = spawn->info->keys; data->key != NULL; data++)
                {
                    ...
                    {
                        Disp(ent, va("%s%s: %s", (color) ? "^3" : "", model_key_description.key,
                                     model_key_description.value));
                    }
                    else if (Q_stricmp(data->key, "#HITBOX") == 0)
                    {
                        Disp(ent, va("%s%s: %s", (color) ? "^3" : "", hitbox_key_description.key,
                                     hitbox_key_description.value));
                    }
                    else
                        Disp(ent, va("%s%s: %s", (color) ? "^3" : "", data->key, data->value));
                    color = !color;
...
gentity_t* G_SpawnGEntityPtrFromSpawnVars(gentity_t* ent, SpawnData_t* spawnData, qboolean inSubBSP)
{
    ...
    static char* gametypeNames[] = {
        "ffa", "holocron", "jedimaster", "duel", "powerduel", "single", "team", "siege", "ctf", "cty", "battleground",
        "saberrun", "reborn"
    };
